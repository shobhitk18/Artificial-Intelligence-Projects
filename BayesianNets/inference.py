...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # Trying to print the values and see what information they are conveying to get a better understanding
        #print(noisyDistance)
        #print(emissionModel)
        #print(pacmanPosition)

        "*** YOUR CODE HERE ***"
        #Dictionary with all keys initialized with 0
        allPossible = util.Counter()                    # Initializing a new object of Counter type which is similar to a dict but all keys initialized with zero counts

        # Replace this code with a correct observation update
        # Be sure to handle the "jail" edge case where the ghost is eaten
        # and noisyDistance is None

        # Following if statement will handle the updating the position of the pacman once it has been eaten to the jail position 
        if noisyDistance == None:                                               # Noisy distance returns None when the ghost has been eaten
            self.setGhostPosition(gameState, self.getJailPosition())
            allPossible[self.getJailPosition()] = 1
        else:
            # Loop over all legal ghost positions and calculate the manhattan distance between pacman and these position
            # If the conditional probability of the noisy distance for the true distance that is supplied is greater than zero 
            # then we update the b elief for that position by multiplying the probability of true distance (self.beliefs[p]) and 
            # the conditional probability (P(noisyDistance | TrueDistance)) to get the final probability.
            for p in self.legalPositions:                                   
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    allPossible[p] = self.beliefs[p] * emissionModel[trueDistance]

        # We should normalize since allPossible contains probability values
        allPossible.normalize()
        #update the self.beliefs with the newly calculated beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # We will now try to figure out. the probabily of each ghost based on its previous positipn
        # and pacmans current position. This is done by gettig the probability distribution of the 
        # likeliness of finding the ghost in each legal position. We woll then use this new distribution
        # to re calaculate our belief values. 
        ...
        for oldPosition in self.legalPositions:
            ...
            olBeliefs = self.beliefs[oldPosition]
            for newPosition, probability in newPosDist.items():
                allPossible[newPosition] = allPossible[newPosition] + (probability * olBeliefs)

        # We will need to normalize this list since it represent a probabilty and shouldn't be 
        # greater than 1. Also normalization is necessary to change back the actual probability to
        # the conditional one.
        allPossible.normalize()
        self.beliefs = allPossible
